-- Naprawa widoku v_thermo_recorder_status po usunięciu kolumny model z tabeli thermo_recorders
...
SELECT 
    ...
    c.valid_until,
    DATEDIFF(c.valid_until, CURRENT_DATE) as days_to_expiry,
    CASE 
        WHEN c.valid_until IS NULL THEN 'NO_CALIBRATION'
        WHEN c.valid_until < CURRENT_DATE THEN 'EXPIRED'
        WHEN DATEDIFF(c.valid_until, CURRENT_DATE) <= 30 THEN 'EXPIRING_SOON'
        ELSE 'VALID'
